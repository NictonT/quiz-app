    ...
    if (storedData) {
        try {
            ...
            if (questions.length > 0) {
                ...
                totalQuestions = randomizedQuestions.length; completedQuestions = 0;
                loadQuestion(); showSlide('quizPage');
            } else {
            ...
            alert(`Error loading quiz questions: ${e.message}\nPlease check the JSON format.`);
             localStorage.removeItem('quizDataToExecute'); goToHomePage();
        }
    } else {
        console.warn("execute.js: No 'quizDataToExecute' found in localStorage.");
        alert("No quiz data found. Please select a quiz file or folder to execute."); goToHomePage();
    }
    updateNightMode();
});
...
function updateNightMode() {
    ...
    if (!nightModeToggle) {
        ...
        document.body.classList.toggle('night-mode', nightModeActive); return;
    }
    const nightModeActive = localStorage.getItem('nightMode') === 'true';
...
function showSlide(slideId) {
    ...
    else { console.error(`execute.js: Slide with ID "${slideId}" not found.`); }
}
...
function loadQuestion() {
    ...
    if (currentIndex >= 0 && currentIndex < randomizedQuestions.length) {
        ...
        for (let displayKey in shuffledDisplayAnswers) {
            ...
            if (!originalKey) { console.warn(`Could not find original key for answer text "${originalAnswerText}" in question ${currentIndex}`); continue; }
...
function selectAnswer(selectedElement) {
     ...
     if (typeof displayKey === 'undefined' || typeof originalKey === 'undefined' || typeof answerText === 'undefined') {
         console.error("Missing data attributes on selected answer element:", selectedElement); return;
     }
     selectedAnswer = { displayKey, originalKey, answerText };
     document.querySelectorAll('.answer').forEach(el => {
         el.classList.remove('selected', 'correct', 'wrong');
         el.style.border = ''; el.style.fontWeight = ''; el.style.opacity = '';
     });
...
function retryQuestion(questionText) {
    ...
    if (originalIndex !== -1) {
         retryingQuestion = true; retryIndex = originalIndex; completedQuestions = originalIndex;
         showSlide('quizPage'); loadQuestion();
    } else { alert("Error: Could not find the selected question to retry."); }
}
...
function redoWrongAnswers() {
    ...
    if (questionsToRedo.length > 0) {
        randomizedQuestions = shuffleArray(questionsToRedo); resetQuizStateForNewRound(); loadQuestion();
    } else { alert("Error finding questions to redo."); }
}

function restartQuiz() {
    if (questions.length === 0) { alert("No questions loaded to restart."); return; }
    randomizedQuestions = shuffleArray([...questions]); resetQuizStateForNewRound(); loadQuestion();
}

function resetQuizStateForNewRound() {
    currentIndex = 0; correctCount = 0; wrongCount = 0; selectedAnswer = null;
    completedQuestions = 0; totalQuestions = randomizedQuestions.length;
    retryingQuestion = false; retryIndex = -1;
    showSlide('quizPage'); updateProgressBar();
}
